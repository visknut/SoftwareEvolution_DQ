...
lrel[int code, value location] serializeAst(set[Declaration] projectAst, int duplicationType) {
	return traverseNodes(projectAst, duplicationType);
}

lrel[int code, value location] traverseNodes(set[Declaration] ast, int duplicationType) {
...
lrel[int code, value location] noSerialize(node x) {
	list[int] ids = [];
	lrel[int code, value location] cS = [];
